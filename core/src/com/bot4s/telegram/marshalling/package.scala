...
object `package` extends CirceEncoders with CirceDecoders with CaseConversions {
  ...
  private def transformKeys(json: Json,
                            f: String => String): Trampoline[Json] = {
    ...
    json.arrayOrObject(
      ...
      _.toList
        .traverse(j => Trampoline.defer(transformKeys(j, f)))
        .map(Json.fromValues(_)),
      transformObjectKeys(_, f)
        .traverse(obj => Trampoline.defer(transformKeys(obj, f)))
        .map(Json.fromJsonObject)
    )
  }

  private def camelKeys(json: io.circe.Json): Json =
    transformKeys(json, camelize).run

  private def snakeKeys(json: io.circe.Json): Json =
    transformKeys(json, snakenize).run
  ...
  def fromJson[T: Decoder](s: String): T = {
    parse(s)
      .fold(throw _, json => camelKeys(json).as[T].fold(throw _, identity))
  }
}

...
class AkkaHttpMarshallingSuite
    extends FunSuite
  ...
  test("Handles AkkaInputFile") {
    ...
    val entity =
      SendDocument(channelId, AkkaInputFile("Pepe", ByteString(content)))
    Post("/", Marshal(entity).to[RequestEntity]) ~> {

...
object AkkaHttpMarshalling {
  ...
  implicit def camelCaseJsonUnmarshaller[R](
    implicit decR: Decoder[R]
  ): FromEntityUnmarshaller[R] = {
    Unmarshaller.stringUnmarshaller
      .forContentTypes(ContentTypes.`application/json`)
      .map(marshalling.fromJson[R])
  }

  implicit def underscore_case_marshaller[T <: Request[_]](
    implicit encT: Encoder[T]
    ...
    Marshaller.strict { request =>
      request match {
        ...
        case r: MultipartRequest[_] =>
            ...
            case (camelKey, inputFile) =>
              ...
              inputFile match {
                case InputFile.FileId(id) =>
                  Multipart.FormData.BodyPart(key, HttpEntity(id))
                ...
                case InputFile.Path(path) =>
                  Multipart.FormData.BodyPart
                    .fromPath(key, MediaTypes.`application/octet-stream`, path)
          ...
          val paramParts = params.map {
            case (key, value) =>
              Multipart.FormData.BodyPart(key, HttpEntity(value))
          }

          Marshalling.Opaque(
            () => Multipart.FormData((parts ++ paramParts): _*).toEntity()
          )
      }
    }
  }
